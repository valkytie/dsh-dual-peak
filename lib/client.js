    ...
    function createDualPeakWidget(ctx) {
      return function DualPeakWidget() {
        ...
        const onDown = (e) => {
          ...
          // Safari 對 mouse 的 pointer 事件在拖曳時會觸發 pointercancel（原生文字選取打斷），
          // pointermove 收不到 → 改用 mouse 事件（桌面瀏覽器 100% 可靠）。
          // 拖曳期間鎖定 body 選取，避免 Safari 啟動文字選取。
        ...
        const card = React.createElement('div', { className: 'dpk-card' },
          React.createElement('div', { className: 'dpk-head', onMouseDown: onDown },
